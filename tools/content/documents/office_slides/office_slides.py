...
class OfficeSlidesTool(BaseTool):
    ...
    title_slide: Optional[Dict[str, str]] = Field(
        None, description="Title slide config with title, subtitle, author"
    )
    charts: Optional[List[Dict[str, Any]]] = Field(
        None, description="Chart definitions to embed in slides"
    )
    ...
    def _validate_parameters(self) -> None:
        ...
        for i, slide in enumerate(self.slides):
            if "title" not in slide:
                raise ValidationError(f"Slide {i} missing 'title' field", tool_name=self.tool_name)
            if "content" not in slide:
                raise ValidationError(
                    f"Slide {i} missing 'content' field", tool_name=self.tool_name
                )
            if "layout" not in slide:
                raise ValidationError(f"Slide {i} missing 'layout' field", tool_name=self.tool_name)
        ...
        if self.title_slide:
            if "title" not in self.title_slide:
                raise ValidationError("title_slide missing 'title' field", tool_name=self.tool_name)
    ...
    def _add_content_slide(
        self, prs: Presentation, slide_def: Dict[str, Any], theme_colors: Dict[str, RGBColor]
    ) -> None:
    ...
    def _add_title_content_slide(
        ...
        # Add content
        content_box = slide.shapes.add_textbox(Inches(0.8), Inches(1.2), Inches(8.4), Inches(3.8))
    ...
    def _add_content_only_slide(
        ...
        content_box = slide.shapes.add_textbox(Inches(0.8), Inches(0.9), Inches(8.4), Inches(4.2))
        content_frame = content_box.text_frame
        content_frame.word_wrap = True
    ...
    def _add_two_column_slide(
        ...
        # Left column
        left_box = slide.shapes.add_textbox(Inches(0.5), Inches(1.2), Inches(4.5), Inches(3.8))
        left_frame = left_box.text_frame
        left_frame.word_wrap = True

        # Right column
        right_box = slide.shapes.add_textbox(Inches(5.0), Inches(1.2), Inches(4.5), Inches(3.8))
