...
class VideoMetadataExtractor(BaseTool):
    ...
    # Parameters
    video_path: str = Field(..., description="Path or URL to the video file", min_length=1)
    extract_thumbnails: bool = Field(
        default=False, description="Extract thumbnail images at key frames"
    )
    include_streams: bool = Field(default=True, description="Include detailed stream information")
    ...
    def _generate_mock_results(self) -> Dict[str, Any]:
        ...
        if self.extract_thumbnails:
            metadata["thumbnails"] = [
                {"timestamp": 0.0, "path": f"{self.video_path}_thumb_00.jpg", "mock": True},
                {"timestamp": 62.75, "path": f"{self.video_path}_thumb_50.jpg", "mock": True},
                {"timestamp": 125.5, "path": f"{self.video_path}_thumb_100.jpg", "mock": True},
            ]
    ...
    def _extract_with_ffprobe(self) -> Dict[str, Any]:
        ...
        if result.returncode != 0:
            raise APIError(f"ffprobe failed: {result.stderr}", tool_name=self.tool_name)
...
if __name__ == "__main__":
    ...
    # Test with mock data
    tool = VideoMetadataExtractor(
        video_path="/path/to/test_video.mp4", extract_thumbnails=True, include_streams=True
    )
    result = tool.run()
